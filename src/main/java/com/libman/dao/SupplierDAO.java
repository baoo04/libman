...
public class SupplierDAO extends DAO {
    ...
    public List<Supplier> searchByName(String name) throws Exception {
        ...
        PreparedStatement ps = conn.prepareStatement("SELECT * FROM tblSupplier WHERE name LIKE ?");
        ps.setString(1, "%" + name + "%");
    ...
    public Supplier findById(int id) throws Exception {
        PreparedStatement ps = conn.prepareStatement("SELECT * FROM tblSupplier WHERE id=?");
        ps.setInt(1, id);
        try (ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
                Supplier s = new Supplier();
